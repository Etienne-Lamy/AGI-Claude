...
- **Mode B « appris »** (`mode_b.py`) : émetteur de programmes AUTORÉGRESSIF à
  émission TYPÉE (masque dur sur le vocabulaire), conditionné par l'OBJECTIF. Appris
  par IMITATION de Mode A. Démontré (étape 13) : le meilleur programme DIFFÈRE selon
  l'objectif (prédire → `predire_champ` ; reconstruire → `compresser→generer`) et
  Mode B émet le bon **sans refaire la recherche** — il amortit Mode A.
...
**NON encore fait de §31** (honnête) : REINFORCE de Mode B (seule l'imitation est là) ;
la bascule mesurée A↔B ; la boucle d'auto-réglage §28.4 branchée en vrai ;
l'instrumentation dashboard §31.10 des nouveaux objets. Résidus : sur-création
occasionnelle des modules-régime (2/régime), rappels conv modérés (~59 %).
