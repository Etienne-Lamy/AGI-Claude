...
Tests : `python3 -m pytest tests/ -q` → **248 passed** (~2 min, plusieurs tests GPU).
...
| 25 | **A\* sur l'état‑objet priorise la branche‑sucre** ✅ | `python3 -m scl.etape25_planif_objet` | `planification_objet.chercher_sucre` (A* sur `(E,v)`, arêtes=accélérations, déroulé unité=traversée du monde). Sur 200 placements : plan trouvé **100 %** (2.4 actions) ; plan exécuté dans le vrai monde → **sucre mangé 84 %** vs **hasard 17 %** (×5). Le 16 % = traversée diagonale approximée |
| 26 | **Orchestrateur à ATTENTION appris d'A\*** ✅ | `python3 -m scl.etape26_orchestrateur_action` | Set Transformer sur jetons‑objets (position + **type** = signal faible) ; imitation de la 1re action d'A* (le prof). Navigue **SANS recherche** : sucre **50 %** (vs A* 94 %, hasard 16 %) — **amortit A***. Ablation : **sans le signal de type → 39 %** → les signaux faibles servent. (50 % < 94 % : le clonage réactif perd la précision multi‑pas ; REINFORCE = suite) |
| 27 | **REINFORCE l'orchestrateur au‑delà de l'imitation** ✅ | `python3 -m scl.etape27_renforce_orchestrateur` | imitation d'A* (démarrage à froid) → REINFORCE sur épisodes réels : **50 %→56 %** (vers A* 94 %). Échec-puis-fix documenté : REINFORCE nu (LR d'imitation, récompense épisode) **dégradait** (50→25 %, oubli de l'expert) → **lot + avantages standardisés + LR dédié** (`pas_renforce_lot`) le stabilise |
| 28 | **Viewer AGENT** (voir percevoir/prédire/planifier/agir) ✅ | `python3 -m scl.demo_agent --log agent.jsonl` + `viewer.py` | l'agent objet en direct : champ **VU vs PRÉDIT‑objet**, objets suivis, vitesse, **plan A\*** en cours, action, sucres/bâtons. Panneau « agent » dans `viewer.html` |
