...
- **Jeu pendant l'ennui (le "rêve")** : PAS bloqué — au contraire. Mesuré cette session : maintenant que la recherche de nourriture fonctionne, l'agent est rassasié la plupart du temps et **`ennui` est le besoin dominant ~97% des pas** (faim ~3%). Le régime de jeu est donc le régime PAR DÉFAUT, actif dès maintenant. Ce qui manque n'est pas l'activation mais la RICHESSE : le comportement d'ennui actuel est un placeholder (`scores_ennui` = récompense du simple déplacement `|v'|`). Prochaine étape à fort effet et immédiatement observable : enrichir la valeur d'action sous ennui en curiosité réelle (recherche de nouveauté / couverture de territoire par navigation à l'estime, ou progrès d'apprentissage §15.2). (Correction d'une lecture erronée antérieure de cette même session qui disait l'inverse — elle datait de la baseline cassée où l'agent ne mangeait pas.)
