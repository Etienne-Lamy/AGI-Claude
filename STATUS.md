...
| 28 | **Viewer AGENT** (voir percevoir/prédire/planifier/agir) ✅ | `python3 -m scl.demo_agent --log agent.jsonl` + `viewer.py` | l'agent objet en direct : champ **VU vs PRÉDIT‑objet**, objets suivis, vitesse, **plan A\*** en cours, action, sucres/bâtons. Panneau « agent » dans `viewer.html`. (étape 27 nuit = REINFORCE/balayage : reste) |
